  ...
  return (
    <PageShell>
      ...
      <Card className="bento p-0">
        <div className="px-5 py-3 border-b border-border/60 text-sm font-semibold">
          Recent invoices
        </div>
          ...
          {invoices.map((inv) => (
            ...
            >
              ...
              <div className="col-span-2">
                <Badge
                  className={`border ${inv.status === "Paid" ? "bg-primary/15 text-primary border-primary/30" : inv.status === "Pending" ? "bg-accent/15 text-accent border-accent/30" : "bg-destructive/15 text-destructive border-destructive/30"}`}
                >
                  {inv.status}
                </Badge>
              </div>
